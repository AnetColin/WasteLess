                ...
                <div style="display:flex; justify-content:space-between; align-items:flex-start;">
                    <div style="display:flex; gap:1rem; align-items:center;">
                        ...
                        <div>
                            ...
                            ${item.forSale ? `<span class="status-badge" style="background:var(--secondary); margin-left:0.5rem;">On Sale: $${item.price || 'Free'}</span>` : ''}
                        </div>
                    </div>
                    <div style="display:flex; gap:0.5rem;">
                        <button onclick="deleteItem(${item.id})" class="icon-btn danger-btn"><i data-lucide="trash-2"></i></button>
                        ${!item.forSale ? `<button onclick="openSellModal(${item.id})" class="icon-btn" style="color:var(--secondary);"><i data-lucide="dollar-sign"></i></button>` : ''}
                    </div>
                </div>
            </div>
            `;
...
window.openSellModal = (id) => {
    const price = prompt("Enter selling price (Enter 0 for Free):");
    if (price !== null) {
        state.items = state.items.map(i => {
            if (i.id === id) return { ...i, forSale: true, price: price };
            return i;
        });
        saveItems();
    }
